...
VecDown::VecDown(Drone& drone) : m_drone{&drone} {}
...
cv::Point2f VecDown::getVecDownDisplacement() const {
    if (!m_hasPrev) {
        throw std::runtime_error(
            "VecDown::getVecDownDisplacement called before calling VecDown::calc");
    }
...
cv::Vec3d VecDown::calcVecDown3d() {
    ...
    cv::Matx33d Rx(1, 0, 0, 0, cos(-gyroData.roll), -sin(-gyroData.roll), 0, sin(-gyroData.roll),
                   cos(-gyroData.roll));

    cv::Matx33d Ry(cos(-gyroData.pitch), 0, sin(-gyroData.pitch), 0, 1, 0, -sin(-gyroData.pitch), 0,
                   cos(-gyroData.pitch));

    cv::Matx33d Rz(cos(-(gyroData.yaw + CV_PI / 2)), -sin((-gyroData.yaw + CV_PI / 2)), 0,
                   sin(-(gyroData.yaw + CV_PI / 2)), cos(-(gyroData.yaw + CV_PI / 2)), 0, 0, 0, 1);
...
cv::Point2f VecDown::calcVecDownProjection() {
    ...
    return {std::max(std::min(u, static_cast<float>(m_drone->cameraInfo.resolutionX)), 0.0f),
