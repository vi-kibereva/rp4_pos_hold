...
VecMove::VecMove(Drone& drone) : m_drone{&drone}, m_vecDown(drone), m_cameraOpticalFlow(drone) {}
...
void VecMove::calc() {
    ...
    if (p.x < 0 || static_cast<int>(p.x) >= m_drone->cameraInfo.resolutionX || p.y < 0 ||
        static_cast<int>(p.y) >= m_drone->cameraInfo.resolutionY) {
    ...
    double yaw = m_drone->getGyroData().yaw + CV_PI / 2;
