...
void signal_handler(int signal) {
  if (signal == SIGINT) {
    std::cout << "\n[SHUTDOWN] Ctrl+C detected, stopping position hold..."
              << std::endl;
    running = false;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cerr << "[ERROR] MSP initialization failed: " << ex.what()
              << std::endl;
    return 1;
  }
  ...
  try {
    ...
    drone = new Drone(*msp, *camera);
    std::cout << "[INIT] Camera started at " << drone->cameraInfo.fps << " FPS"
              << std::endl;
  } catch (const std::exception &ex) {
    std::cerr << "[ERROR] Drone initialization failed: " << ex.what()
              << std::endl;
    delete msp;
    return 1;
  }
  ...
  try {
    ...
    std::cerr << "[ERROR] VecMove initialization failed: " << ex.what()
              << std::endl;
    delete drone;
    delete msp;
    return 1;
  ...
  std::cout << "[INIT] Waiting 2 seconds for system stabilization..."
            << std::endl;
  ...
  while (running) {
    ...
    try {
      ...
      msp::SetRawRcData rc_command(roll_pid, pitch_pid, THROTTLE_VALUE,
                                   YAW_VALUE);
      msp->setRawRc(rc_command);

      start = clock::now();
      float timestamp = frame_count / static_cast<float>(CONTROL_RATE_HZ);
      telemetry_log << timestamp << "," << current_position.x << ","
                    << current_position.y << "," << target_position.x << ","
                    << target_position.y << "," << velocity.x << ","
                    << velocity.y << "," << roll_pid << "," << pitch_pid << ","
                    << current_aux3 << "," << (aux3_active ? 1 : 0)
                    << std::endl;
  ...
  try {
    ...
    std::cerr << "[WARNING] Could not send shutdown commands: " << ex.what()
              << std::endl;
  }
  ...
  std::cout << "[SHUTDOWN] Position hold terminated. Total frames: "
            << frame_count << std::endl;
  std::cout << "[SHUTDOWN] Telemetry saved to: position_hold_telemetry.csv"
            << std::endl;
