...
AttitudeData Msp::attitude() {
    ...
    if (!bitaflught_msp_.request(MSP_ATTITUDE, payload, EXPECTED_SIZE, &recv_size)) {
...
StatusData Msp::status() {
    ...
    if (!bitaflught_msp_.request(MSP_STATUS, payload, sizeof(payload), &recv_size)) {
...
AltitudeData Msp::altitude() {
    ...
    if (!bitaflught_msp_.request(MSP_ALTITUDE, payload, EXPECTED_SIZE, &recv_size)) {
...
RawImuData Msp::rawImu() {
    ...
    if (!bitaflught_msp_.request(MSP_RAW_IMU, payload, EXPECTED_SIZE, &recv_size)) {
